...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-100">Edit Product</h1>
    </div>

    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700 max-w-2xl">
        <form action="{{ route('admin.merchandise.update', $product->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            @if ($product->photo)
                <div>
                    ...
                    <img src="{{ asset('storage/' . $product->photo) }}" class="w-32 h-32 object-cover rounded-lg"
                        alt="">
                </div>
            @endif

            <div>
                <label for="photo" class="block text-sm font-medium text-gray-300 mb-2">New Photo (optional)</label>
