...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-100">Edit Kegiatan: {{ $kegiatan->activity }}</h1>
    </div>

    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700 max-w-2xl">
        <form action="{{ route('admin.kegiatan.update', $kegiatan->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="time" id="time"
                        ...
                        value="{{ old('time', $kegiatan->time) }}" placeholder="08:00 - 09:00" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order"
                        class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        value="{{ old('order', $kegiatan->order) }}" min="1" required>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="activity" id="activity"
                    class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    value="{{ old('activity', $kegiatan->activity) }}" placeholder="e.g., Opening Ceremony" required>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="icon" id="icon"
                        class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        value="{{ old('icon', $kegiatan->icon) }}" placeholder="e.g., fas fa-music">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="color" name="color_picker" id="color_picker"
                            ...
                            value="{{ old('color', $kegiatan->color ?: '#6366f1') }}"
                        ...
                        <input type="text" name="color" id="color"
                            ...
                            value="{{ old('color', $kegiatan->color ?: '#6366f1') }}" placeholder="#6366f1">
                    </div>
                </div>
            </div>
